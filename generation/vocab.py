...
class VocabEntry(object):
    ...
    def from_bilingual_corpus(src_corpus, trg_corpus, src_size, trg_size, remove_singleton=True):
        ...
        for word in top_trg_words:
            if len(vocab_entry) < src_size + trg_size:
...
class Vocab(object):
    def __init__(self, src_sents=None, trg_sents=None, src_vocab_size=50000, trg_vocab_size=50000, remove_singleton=True, share_vocab=False):
        ...
        if src_sents is not None and trg_sents is not None:
            if share_vocab: # my project on hncm (seq2seq for dialogs) belongs to this category
                ...
                self.src = self.trg = self.share # src = trg = share which means only use the share vocab
            else:
...
if __name__ == '__main__':
    ...
    vocab = Vocab(src_sents, trg_sents, args.src_vocab_size, args.trg_vocab_size, remove_singleton=not args.include_singleton, share_vocab=args.share_vocab)
