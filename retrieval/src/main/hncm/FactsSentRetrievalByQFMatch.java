...
public class FactsSentRetrievalByQFMatch {

public static void main(String[] args) throws IOException, ParseException{
		//java -jar factsSentRetrievalByQFMatch.jar 50 sentText udc
		if(args.length < 3){
		    	System.out.println("please input params: hitsPerPage(100) searchFieldName(sentText) dataName(udc)");
		    	System.exit(1);
	    }
		int hitsPerPage = Integer.valueOf(args[0]);
	    String searchFieldInPosts = args[1], dataName = args[2];// the dataName could be udc
	    System.out.println("input params hitsPerPage searchFieldName dataName: " + hitsPerPage + "\t" + searchFieldInPosts + "\t" + dataName);
		String modelInputDataPath = "/net/home/lyang/NLPIRNNHybridNCMRetGen/src-hybrid-ncm/data/" + dataName +"/ModelInput/";
		String indexPath = "data/ExternalCollection/askubuntu/askubuntu-index-qandasents-whole/";
		...
		for (String dataPart : new String[]{"dev", "test", "train"}){
			...
			String factsFile = modelInputDataPath + dataPart + "_fact.txt"; // retrieved facts
			//The data format of the UDC facts/tips file is
			//id \t context \t fact_1 \t fact_2 \t … \t fact_n \t response
